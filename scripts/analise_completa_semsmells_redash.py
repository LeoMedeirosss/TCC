...
def find_react_components(repo_path, exclude_dirs=None, exclude_files=None):
    ...
    if exclude_dirs is None:
        exclude_dirs = {'node_modules', 'build', 'dist', 'test', 'tests', 'migrations', 'config'}
...
def analyze_clean_components(repo_path, output_file, base_dir, sample_size=20):
    ...
    with open(str(base_dir / 'analises' / 'redash' / 'components_smells_redash.csv'), 'r', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    repo_path = base_dir / "redash"
    output_file = base_dir / "analises" / "redash" / "analise_redash_clean_components.txt"
